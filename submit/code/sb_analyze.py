...
def t_or_u(x, y, norm_test=stats.shapiro, verbose=0, dependent=False):
    ...
    if norm_test is not None:
        ...
        t_ok = not is_significant(
            x_ntresult[1], 0.05) and not is_significant(y_ntresult[1], 0.05)
    else:
        t_ok = True
    ...
    if t_ok:
        ...
        if dependent:
            ...
            effect_size = testresult[0] / (nobs1**0.5)
        else:
            ...
            # effect_size = testresult[0] * (nobs1*nobs2/(nobs1+nobs2))**0.5
...
def main():
    ...
    if args.plot:
        ...
        burger_values = {desc[1]: value for desc, value in df_plot
                         if desc[0] == 'burger'}

        swipe_values = {desc[1]: value for desc, value in df_plot
                        if desc[0] == 'swipe'}
