...
instruction is displayed. The distance to cover from one question to another is always in the same small range for each puzzle. However, the distances differ for
different puzzles. As an example, consider the example in Figure \ref{fig:puzzle} which has distances in range two to three. The arrows indicate the question to jump to in
...
For our study, we decided for a between-group design for three reasons. First, fatigue may be an issue because, although the tasks are rather simple, they require the participant
to keep the focus in order to complete them successfully. Second, when solving the tasks with the second type, the test persons might rate their satisfaction more drastically, as they
are biased from having solved the puzzles with the respective other navigation method. Ultimately, in order to avoid an extreme learning effect, the puzzles for swipe gesture navigation and menu navigation
