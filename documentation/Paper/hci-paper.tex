...
length five by using distances $d$ three times and distance $d - 1$ twice. Hence, on average we have distances $\frac{3 * d + 2 * (d - 1)}{5} = \frac{5d - 2}{5}
= d - \frac{2}{5}$, which we will work with $d$ in the remainder of the paper. Furthermore, the distances limited to two 
neighbored values, it is reasonable to assume negligible impact on the user-satisfaction and time.
